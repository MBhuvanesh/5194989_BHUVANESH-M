...
int* waiter(int number_count, int* number, int q, int* result_count) {
    int primes[10000], pc = 0, limit = 10000;
    bool isPrime[10001];
    ...
    memcpy(A, number, number_count * sizeof(int));
    ...
    for (int i = 0; i < q; i++) {
        ...
        for (int j = A_size - 1; j >= 0; j--) {
            ...
            else nextA[nextA_size++] = A[j];
        }

        for (int j = B_size - 1; j >= 0; j--) res[res_idx++] = B[j];
    ...
    for (int j = A_size - 1; j >= 0; j--) res[res_idx++] = A[j];
